...
(async () => {
  ...
  if (!admin) { console.log("KHONG co admin key", keys.status, keys.body.slice(0, 200)); return; }
  ...
  for (const [model, note] of cases) {
    const r = await req("POST", "/v1/chat/completions", A, { model, messages: [{ role: "user", content: "hi" }] });
    let code = ""; try { code = JSON.parse(r.body)?.error?.code || ""; } catch {}
