...
(async () => {
  ...
  console.log(
    ...
    s.strictModelResolution,
    '| defaultProvider =',
    s.defaultProvider,
    '| enableSmartFallback =',
    s.enableSmartFallback,
  );

  const nonce = Math.random().toString(36).slice(2, 8);
  const r = await call('POST', '/v1/chat/completions', K, {
    model: 'xyz-khong-ton-tai-' + nonce,
  ...
  const r2 = await call('POST', '/v1/chat/completions', K, {
    model: 'wenker-cloud/wenker-deepseek-v3-free',
    messages: [{ role: 'user', content: `ma ${n2}, 2+3?` }],
    stream: false,
  });
  console.log(
    `\n[wenker-cloud xkiro] HTTP ${r2.status} content=`,
    (r2.json?.choices?.[0]?.message?.content || '').slice(0, 40),
  );
})();
