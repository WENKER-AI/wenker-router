...
(async () => {
  ...
  check("hostFromInput(Request)", proxyFetch.hostFromInput(new Request("http://api.openai.com/v1")) === "api.openai.com");
  ...
  check("bypass noProxy list", proxyFetch.shouldBypass("intranet.corp", ["intranet.corp"]) === true);
  ...
  let directErr = null;
  try { await fetch("http://public.example/x"); } catch (e) { directErr = e; }
  check("proxy tat: public host khong di qua proxy (loi DNS thang)", directErr !== null, directErr && directErr.cause && directErr.cause.code);

  // ---- 3. Bat proxy: public host -> QUA PROXY; private host -> THANG ----
  db.updateSettings({ proxyEnabled: true, proxyUrl, proxyNoProxy: "localhost" });
  const pub = await fetch("http://public.example/proxied", { signal: AbortSignal.timeout(5000) }).catch((e) => ({ __err: e }));
  if (pub.__err) { check("proxy bat: public host di QUA proxy", false, "TIMEOUT/ERR: " + pub.__err.message); }
  else {
    const pubBody = await pub.text();
    check("proxy bat: public host di QUA proxy", pub.headers.get("x-via-proxy") === "1" && pubBody === "via-proxy:/proxied", pubBody);
  }

  const loop = await fetch(`http://127.0.0.1:${PORT}/direct`, { signal: AbortSignal.timeout(5000) });
  const loopBody = await loop.text();
  check("proxy bat: loopback van di THANG (x-via-proxy:0)", loop.headers.get("x-via-proxy") === "0" && loopBody === "direct:/direct", loopBody);

  check("fake proxy thay request absolute-form", seen.proxied.some((u) => /public\.example/.test(u)));
  ...
  const withOwn = await fetch("http://public.example/own", { dispatcher: own, signal: AbortSignal.timeout(5000) }).catch(() => ({ headers: new Map() }));
  check("giu dispatcher cua caller", withOwn.headers.get("x-via-proxy") === "1");
  try { own.close(); } catch (e) {}
  ...
  check("testProxy chan URL sai", tbad.ok === false && /khong hop le|not|http/i.test(tbad.error), tbad.error);

  // ---- 6. reset() khong crash, pick up URL moi ----
  proxyFetch.reset();
  const afterReset = await fetch("http://public.example/after", { signal: AbortSignal.timeout(5000) }).catch(() => ({ headers: new Map() }));
  check("sau reset() van di qua proxy (agent duoc tao lai)", afterReset.headers.get("x-via-proxy") === "1");
