...
function logo(p) { const h = host(p.website || p.baseUrl || ''); if (!h) return ''; return `<img src="https://icons.duckduckgo.com/ip3/${h}.ico" alt="" height="20" width="20" />`; }
function auth(p) { if (!p.requiresAuth) return '免密 No-key'; return (p.authType || 'key').toString(); }
const byCat = {};
