...
const REFUSE = /reached its budget|payment required|rate limit|too many requests|quota|invalid api key|no healthy upstream/i;
async function one(m) {
  const r = await post({ model: m, messages: [{ role: 'user', content: 'Chi tra loi dung 1 tu: Trai dat hinh gi?' }], stream: false, max_tokens: 20 });
  ...
  console.log(kind.padEnd(9) + m.padEnd(42) + String(r.ms).padStart(6) + 'ms  served=' + (r.j && r.j.model) + '  "' + c.replace(/\s+/g, ' ').slice(0, 50) + '"' + (r.st !== 200 ? '  ' + r.s.slice(0, 80) : ''));
}
(async () => {
  const list = [
  ...
  for (const m of list) { await one(m); await new Promise((r) => setTimeout(r, 800)); }
})();
