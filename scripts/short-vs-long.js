...
async function main() {
  ...
  const LONG =
    'Giải thích bằng tiếng Việt thật ngắn gọn thế nào để một router AI local ' +
    'chuyển yêu cầu OpenAI-compatible sang nhiều upstream miễn phí, có circuit breaker, ' +
    'rate limit và cache câu trả lời. ' +
    n() +
    ' Hãy viết khoảng 200 từ.';
  for (const m of models) {
    ...
    console.log(
      'NGAN  ' +
        ...
        short.kind.padEnd(8) +
        ' pt=' +
        String(short.pt).padStart(4) +
        ' ' +
        String(short.ms).padStart(6) +
        'ms  served=' +
        short.served +
        '  "' +
        short.txt +
    ...
    console.log(
      'DAI   ' +
        ...
        lng.kind.padEnd(8) +
        ' pt=' +
        String(lng.pt).padStart(4) +
        ' ' +
        String(lng.ms).padStart(6) +
        'ms  served=' +
        lng.served +
        '  "' +
        lng.txt +
