...
(async () => {
  ...
  if (!admin) { console.log("KHONG co admin key"); return; }
  ...
  let mj; try { mj = JSON.parse(models.body); } catch { mj = { data: [] }; }
  const list = mj.data || [];
  ...
  for (const pid of targets) {
    ...
    console.log(`   -> ${content ? JSON.stringify(String(content).slice(0, 90)) : String(err || r.body).replace(/\s+/g, " ").slice(0, 150)}`);
    console.log();
  }
