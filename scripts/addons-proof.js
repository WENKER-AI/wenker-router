...
(async () => {
  ...
  check(
    'GET /api/addons',
    list.status === 200 && addons.length >= 6,
    `${addons.length} add-on, status ${list.status}`,
  );
  check(
    'moi theme co `resolved` CSS vars',
    addons
      .filter((a) => a.type === 'theme')
      .every((a) => a.resolved && Object.keys(a.resolved).length > 20),
    `theme keys=${Object.keys((addons.find((a) => a.type === 'theme') || {}).resolved || {}).length}`,
  );
  check(
    'midnight khong doi mau goc',
    JSON.stringify(
      addons.find((a) => a.id === 'wenker.theme.midnight')?.resolved?.['--color-cyan-500'],
    ) === JSON.stringify('6 182 212'),
    'cyan-500=' +
      addons.find((a) => a.id === 'wenker.theme.midnight')?.resolved?.['--color-cyan-500'],
  );
  ...
  check(
    'POST /api/addons/install (2 mau)',
    inst.status === 200 && inst.data.success,
    `resolved=${Object.keys(inst.data?.addon?.resolved || {}).length} bien`,
  );
  check(
    'radius duoc giu nguyen',
    inst.data?.addon?.resolved?.['--radius'] === '16px',
    'radius=' + inst.data?.addon?.resolved?.['--radius'],
  );
  check(
    'sinh du 11 shade accent',
    Object.keys(inst.data?.addon?.resolved || {}).filter((k) => k.startsWith('--color-cyan-'))
      .length === 11,
  );

  const after = await j('GET', '/api/addons');
  check(
    'add-on moi xuat hien trong kho',
    (after.data.addons || []).some((a) => a.id === manifest.id),
  );
  ...
  const bad2 = await j('POST', '/api/addons/install', {
    source: JSON.stringify({ id: 'a.b', name: 'x', type: 'theme', theme: {} }),
  });
  check('chan theme khong co mau', bad2.status === 400, bad2.data?.error);
  const bad3 = await j('POST', '/api/addons/install', {
  ...
  const bad4 = await j('POST', '/api/addons/install', {
    source: JSON.stringify({
      id: 'evil.x',
      name: 'x',
      type: 'theme',
      theme: { '--color-cyan-500': '6 182 212; } body { display:none' },
    }),
  });
  check('chan gia tri CSS tieu cuc', bad4.status === 400, bad4.data?.error);
  const bad5 = await j('POST', '/api/addons/install', {
    source: JSON.stringify({
      id: 'evil.y',
      name: 'x',
      type: 'theme',
      theme: { '--bg-primary': 'url(javascript:alert(1))' },
    }),
  });
  ...
  const prov = await j('POST', '/api/addons/install', {
    source: JSON.stringify({
      ...
      provider: {
        ...
        requiresAuth: false,
        models: [{ id: 'proof-model', name: 'Proof Model', contextWindow: 4096 }],
      },
    }),
  });
  ...
  check(
    'provider moi xuat hien trong /v1/models',
    Boolean(proof),
    proof ? `id=${proof.id} status=${proof.wenker_status}` : 'khong thay',
  );
  check(
    '/v1/models day du metadata finder',
    Boolean(proof && proof.wenker_category && 'wenker_free' in proof && 'wenker_context' in proof),
    proof
      ? `cat=${proof.wenker_category} free=${proof.wenker_free} ctx=${proof.wenker_context}`
      : '',
  );
  ...
  check(
    'catalog > 200 model (doc that tu localhost)',
    all.length > 200,
    `${all.length} model, ${withCtx} co context, ${free} mien phi`,
  );
