...
(async () => {
  ...
  for (const m of ["wenker-cloud/wenker-deepseek-v3-free", "wenker-cloud/mistralai/devstral-medium", "wenker-cloud/mistralai/ministral-3b"]) {
    const n = Math.random().toString(36).slice(2, 9);
    const r = await call("POST", "/v1/chat/completions", K, { model: m, messages: [{ role: "user", content: `ma ${n}: 5+5?` }], stream: false });
    console.log(`HTTP ${r.status}  ${m.padEnd(42)} ${await latest()}\n        -> ${txt(r.json)}`);
  }
  const mm = await call("GET", "/v1/models", K, null);
  console.log("\nwenker-cloud/* trong /v1/models =", (mm.json?.data || []).filter((x) => x.id.startsWith("wenker-cloud/")).length);
  console.log("xkiro-basic/* con trong /v1/models =", (mm.json?.data || []).some((x) => x.id.startsWith("xkiro-basic/")));
