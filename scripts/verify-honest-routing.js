...
async function main() {
  ...
  console.log(
    ...
    JSON.stringify({
      id: alias?.id,
      display: alias?.wenker_display_name,
      served_by: alias?.wenker_served_by,
    }),
  );
  ...
  const r2 = await fetch(`${BASE}/v1/chat/completions`, {
    ...
    body: JSON.stringify({
      model: 'github-models/DeepSeek-R1',
      messages: [{ role: 'user', content: 'hi' }],
    }),
  });
  const j2 = await r2.json();
  console.log(
    '[2] github-models (no key):',
    r2.status,
    j2.error?.code,
    '|',
    (j2.error?.message || '').slice(0, 90),
  );
  ...
  const r4 = await fetch(`${BASE}/v1/chat/completions`, {
    ...
    body: JSON.stringify({
      model: 'wenker-cloud/wenker-deepseek-r1-free',
  ...
  console.log(
    '[4] wenker-cloud alias:',
    r4.status,
    '| served_by:',
    j4.wenker_served_by,
    '| content:',
    JSON.stringify((j4.choices?.[0]?.message?.content || '').slice(0, 40)),
  );
  ...
  for (const l of (Array.isArray(logs) ? logs : logs.logs || []).slice(0, 6)) {
    console.log(
      ...
      '->',
      'prov=' + l.providerId,
      'served=' + l.resolvedModel,
      'status=' + l.status,
      l.fallbackFrom ? `FB from ${l.fallbackFrom}` : '',
    );
  }
}
main().catch((e) => {
