...
const REFUSAL =
  /reached its budget|insufficient balance|requires real deposited|top up|raise the budget/i;
async function chat(A, model) {
  const nonce = Math.random().toString(36).slice(2, 8);
  const r = await call('POST', '/v1/chat/completions', A, {
    model,
    messages: [{ role: 'user', content: 'Reply with exactly one word: ' + nonce }],
    max_tokens: 25,
  });
  ...
  return {
    ...
    status: r.status,
    content: c
      ? String(c).replace(/\s+/g, ' ').slice(0, 36)
      : String(e || r.body)
        .replace(/\s+/g, ' ')
        .slice(0, 90),
  };
...
(async () => {
  ...
  console.log(
    ...
    ids.length,
    '| xkiro-basic:',
    ids.filter((x) => x.startsWith('xkiro-basic/')).length,
    '| wenker-vip:',
    ids.filter((x) => x.startsWith('wenker-vip/')).length,
  );
  console.log(
    '  xkiro mau:',
    ids
      .filter((x) => x.startsWith('xkiro-basic/'))
      .slice(0, 3)
      .join(', '),
  );
  ...
  const an = await call(
    'POST',
    '/v1/messages',
    Object.assign({ 'anthropic-version': '2023-06-01' }, A),
    {
      model: 'xkiro-basic/mistralai/codestral-2508',
      max_tokens: 25,
  ...
  console.log(
    `  HTTP ${an.status}  ${txt ? 'OK-THAT ' + JSON.stringify(txt.slice(0, 30)) : 'LOI ' + String(an.json?.error?.message || an.body).slice(0, 80)}`,
  );
})();
