...
function call(method, path, headers, bodyObj) {
  return new Promise((resolve) => {
    ...
    const r = http.request({ host: "127.0.0.1", port: 3600, path, method, headers: h },
      (res) => { let b = ""; res.on("data", (c) => (b += c)); res.on("end", () => { let j; try { j = JSON.parse(b); } catch { j = b; } resolve({ status: res.statusCode, json: j, body: b }); }); });
...
if (!IZZI || !XKIRO) { console.error("THIEU bien WK_IZZI_KEY / WK_XKIRO_KEY"); process.exit(2); }

// 10 model da duoc chung minh SONG qua xkiro (HTTP 200 co content) khi quet toan bo 83 model.
const XKIRO_MODELS = [
  "mistralai/codestral-2508", "mistralai/mistral-large-2512", "mistralai/mistral-medium-3.5",
  "mistralai/mistral-small-2603", "mistralai/devstral-medium", "mistralai/ministral-14b",
  "mistralai/ministral-8b", "mistralai/ministral-3b", "sensenova/sensenova-6.8-flash-lite",
...
const IZZI_MODELS = [
  { id: "wenker-vip-deepseek", name: "WENKER VIP DeepSeek V4 Flash", contextWindow: 64000, isFree: false, targetModel: "deepseek-v4-flash" },
  { id: "wenker-vip-gpt4o-mini", name: "WENKER VIP GPT-4o mini", contextWindow: 128000, isFree: false, targetModel: "gpt-4o-mini" },
  { id: "wenker-vip-gemini-flash", name: "WENKER VIP Gemini 2.5 Flash", contextWindow: 1000000, isFree: false, targetModel: "gemini-2.5-flash" },
];
...
(async () => {
  ...
  if (!hasXkiro) {
    const r = await call("POST", "/api/providers/custom/add", A, {
      id: "xkiro-basic", name: "WENKER Basic (xkiro)", baseUrl: "https://api.xkiro.com/v1",
      authType: "bearer", userApiKey: XKIRO, isFree: true, requiresAuth: true,
      website: "https://xkiro.com", description: "Nha cung cap basic nhieu model, thay the Pollinations cho WENKER Cloud.",
      models: XKIRO_MODELS,
    });
    console.log("tao xkiro-basic ->", r.status, r.json?.success ? "OK" : r.body);
  } else {
    const u = await call("POST", "/api/providers/xkiro-basic", A, { userApiKey: XKIRO, models: XKIRO_MODELS, enabled: true });
  ...
  const v = await call("POST", "/api/providers/wenker-vip", A, {
    baseUrl: "https://api.izziapi.com/v1", authType: "bearer", userApiKey: IZZI,
    requiresAuth: true, enabled: true, models: IZZI_MODELS,
  ...
  const s = await call("POST", "/api/settings", A, {
    ...
    fallbackOrder: ["openrouter", "xkiro-basic", "wenker-vip", "wenker-cloud", "groq", "google-gemini", "nim-nvidia", "duckduckgo", "pollinations"],
  });
  ...
  for (const id of ["xkiro-basic", "wenker-vip", "wenker-community"]) {
    ...
    console.log(`  [${id}] enabled=${p?.enabled} baseUrl=${p?.baseUrl} hasKey=${p?.hasApiKey} models=${(p?.models || []).length}`);
  }
})();
