...
function orPublic() {
  return new Promise((resolve) => {
    const r = https.request({ host: "openrouter.ai", path: "/api/v1/models", method: "GET", timeout: 40000 },
...
(async () => {
  ...
  try {
    ...
    freeSlugs = ms.filter((m) => {
      const p = m.pricing || {};
      return Number(p.prompt) === 0 && Number(p.completion) === 0;
    }).map((m) => m.id);
    console.log(`OpenRouter /models: HTTP ${pub.status}, tong ${ms.length} model, ${freeSlugs.length} model gia 0 (that su mien phi)`);
  } catch (e) { console.log("khong doc duoc /models:", pub.status, pub.body.slice(0, 120)); }
  ...
  for (const m of candidates) {
    ...
    console.log(`${r.status === 200 && c ? "SONG " : "CHET "} ${m.padEnd(58)} HTTP ${r.status}  ${c ? JSON.stringify(String(c).slice(0, 45)) : String(e || r.body).replace(/\s+/g, " ").slice(0, 110)}`);
  }
