...
(async () => {
  ...
  for (const id of ["wenker-cloud", "wenker-vip", "wenker-community", "xkiro-basic", "pollinations"]) {
    const x = list.find((y) => y.id === id);
    if (!x) { console.log(`${id}: KHONG CO`); continue; }
    console.log(`${id}: name="${x.name}" cat=${x.category} enabled=${x.enabled} baseUrl=${x.baseUrl} auth=${x.authType} hasKey=${x.hasApiKey} models=${(x.models||[]).length}`);
