...
(async () => {
  ...
  await call(
    ...
    '/api/settings',
    A,
    Object.assign({}, settings, { enableSmartFallback: false }),
  );
  ...
  const latestProvider = async () => {
    ...
    return e
      ? `provider=${e.providerId} model=${e.resolvedModel || e.model} cached=${!!e.cached} status=${e.status}`
      : 'khong co log';
  };
  ...
  const t = async (model) => {
    ...
    const r = await call('POST', '/v1/chat/completions', K, {
      messages: [{ role: 'user', content: `Ma ${nonce}. Tra loi ngan: 1+1 bang may?` }],
      stream: false,
      model,
    });
    const who = await latestProvider();
    const tag =
      r.status === 200 && txt(r.json) ? 'SONG' : r.status === 200 ? '200-RONG' : 'LOI-' + r.status;
    console.log(
      `${tag.padEnd(9)} ${model.padEnd(34)} ${who}\n            noi dung: ${txt(r.json) || err(r.json)}`,
    );
  };
  ...
  await call(
    ...
    '/api/settings',
    A,
    Object.assign({}, settings, { enableSmartFallback: true }),
  );
