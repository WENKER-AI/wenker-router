...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (payload.stream) {
      ...
      res.write(
        `data: ${JSON.stringify({ id: 'm', object: 'chat.completion.chunk', choices: [{ index: 0, delta: { role: 'assistant' } }] })}\n\n`,
      );
      res.write(
        `data: ${JSON.stringify({ id: 'm', object: 'chat.completion.chunk', choices: [{ index: 0, delta: { content } }] })}\n\n`,
      );
      res.write('data: [DONE]\n\n');
      res.end();
    } else {
...
(async () => {
  ...
  await api('/api/providers/custom/add', {
    method: 'POST',
    body: JSON.stringify({
      id: 'mockprov',
      name: 'Mock Provider',
      ...
      requiresAuth: false,
      models: [{ id: 'mock-echo', name: 'Mock', contextWindow: 4096, isFree: true }],
    }),
  });
  ...
  // A. Cline-style prefixed model, NON-stream.
  {
    const r = await api('/v1/chat/completions', {
      method: 'POST',
      body: JSON.stringify({
        model: 'openai-compatible:mockprov/mock-echo',
        messages: [{ role: 'user', content: 'A ' + Date.now() }],
        stream: false,
      }),
    });
    out.push([
      'A prefix non-stream',
      `status=${r.status} content=${JSON.stringify(r.j?.choices?.[0]?.message?.content)}`,
    ]);
  }
  ...
  // B. Cline-style prefixed model, STREAM (SSE passthrough).
  {
    const r = await fetch(B + '/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'openai-compatible:mockprov/mock-echo',
        messages: [{ role: 'user', content: 'B ' + Date.now() }],
        stream: true,
      }),
    });
    ...
    out.push([
      'B prefix stream',
      `status=${r.status} sseDone=${hasDone} sseHasContent=${hasContent} bytes=${text.length}`,
    ]);
  }
  ...
  // C. Anthropic /v1/messages path (what Cline/Claude Code actually calls).
  {
    const r = await api('/v1/messages', {
      method: 'POST',
      body: JSON.stringify({
        model: 'mockprov/mock-echo',
        max_tokens: 64,
        messages: [{ role: 'user', content: 'C ' + Date.now() }],
      }),
    });
    out.push([
      'C /v1/messages',
      `status=${r.status} text=${JSON.stringify(r.j?.content?.[0]?.text ?? r.j?.error?.message ?? r.t.slice(0, 60))}`,
    ]);
  }
