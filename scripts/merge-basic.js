...
(async () => {
  ...
  if (!cloud) {
    console.error('khong thay wenker-cloud');
    process.exit(1);
  }
  ...
  console.log(
    `wenker-cloud: ${cloud.models.length} model + xkiro ${raw.length} slug -> ${merged.length} sau gop`,
  );

  const r1 = await call('POST', '/api/providers/wenker-cloud', A, {
    models: merged,
    description:
      'WENKER Cloud (basic) - nguon xkiro, DA GOP \'WENKER Basic (xkiro)\' vao day. Ten wenker-*-free la ALIAS; slug mistralai/* la ten that cua xkiro. Xem targetModel o chi tiet.',
  });
  ...
  const r2 = await call(
    ...
    '/api/settings',
    A,
    Object.assign({}, s, { fallbackOrder: order, defaultProvider: 'wenker-cloud' }),
  );
