...
function startMock() {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      ...
      req.on("end", () => {
        let body = {};
        try { body = JSON.parse(raw); } catch (e) { /* keep empty */ }
        ...
        if (req.url.endsWith("/chat/completions")) {
          ...
          seen.toolsPerCall.push(Array.isArray(body.tools) ? body.tools.map((t) => t.function && t.function.name) : null);

          if (last.role === "tool") {
            // Nuoc 2: history phai du (assistant.tool_calls + tool.tool_call_id).
            const assistantWithCalls = msgs.find((m) => m.role === "assistant" && Array.isArray(m.tool_calls) && m.tool_calls.length);
            const toolReply = msgs.find((m) => m.role === "tool" && m.tool_call_id);
            if (!assistantWithCalls) mockFailures.push("nuoc 2 thieu assistant.tool_calls trong history");
            ...
            return res.end(JSON.stringify({
              id: "mock-2", object: "chat.completion", created: 1,
              model: body.model,
              choices: [{ index: 0, message: { role: "assistant", content: "TOOL_RESULT_OK: " + (toolReply ? toolReply.content : "?") }, finish_reason: "stop" }],
          ...
          return res.end(JSON.stringify({
            id: "mock-1", object: "chat.completion", created: 1,
            model: body.model,
            choices: [{
              index: 0,
              message: {
                role: "assistant", content: null,
                tool_calls: [{ id: "call_abc123", type: "function", function: { name: "read_file", arguments: JSON.stringify({ path: "a.txt" }) } }]
              },
        ...
        if (req.url.endsWith("/models")) {
          return res.end(JSON.stringify({ object: "list", data: [{ id: "mock-model", object: "model", owned_by: "mock" }] }));
        }
        res.statusCode = 404;
...
function startRouter() {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(__dirname, "..", "server", "index.js")], {
      env: { ...process.env, PORT: String(ROUTER_PORT), HOST: "127.0.0.1", WENKER_HOME: path.join(process.env.TEMP || "/tmp", "wenker-tools-test") },
...
(async () => {
  ...
  try {
    ...
    const add = await api("/api/providers/custom/add", {
      method: "POST",
      body: JSON.stringify({
        id: PROVIDER_ID, name: "Mock Tools", baseUrl: `http://127.0.0.1:${MOCK_PORT}/v1`,
        authType: "none", requiresAuth: false, isFree: true,
        models: [{ id: "mock-model", name: "Mock Model", contextWindow: 32000, isFree: true }]
      })
    });
    check("them custom provider", add.status === 200 && add.body && add.body.success, add.body && add.body.error);

    // Nuoc 1: gui tools -> phai nhan ve tool_calls.
    const c1 = await api("/v1/chat/completions", {
      method: "POST",
      body: JSON.stringify({ model: "mock-model", messages: [{ role: "user", content: "doc a.txt" }], tools: TOOLS })
    });
    const m1 = c1.body && c1.body.choices && c1.body.choices[0].message;
    check("nuoc 1: HTTP 200", c1.status === 200, c1.body && c1.body.error);
    check("nuoc 1: router tra duoc tool_calls", Boolean(m1 && Array.isArray(m1.tool_calls) && m1.tool_calls.length === 1), m1);
    check("nuoc 1: finish_reason = tool_calls", c1.body && c1.body.choices[0].finish_reason === "tool_calls");
    check("nuoc 1: upstream THAY duoc schema `tools`",
      seen.toolsPerCall[0] && seen.toolsPerCall[0].includes("read_file"), seen.toolsPerCall[0]);
    ...
    check("nuoc 2: upstream thay assistant.tool_calls + tool.tool_call_id",
      seen.messagesInSecondCall && seen.messagesInSecondCall.assistantToolCalls && seen.messagesInSecondCall.toolCallId === "call_abc123",
      seen.messagesInSecondCall);
    check("nuoc 2: cau tra loi cuoi chua ket qua tool",
      typeof finalContent === "string" && finalContent.includes("noi dung file a.txt"), finalContent);

    // Chan doan trung thuc: model Pollinations + tools -> 400 tools_not_supported.
    const c3 = await api("/v1/chat/completions", {
      method: "POST",
      body: JSON.stringify({ model: "wenker-deepseek-r1-free", messages: [{ role: "user", content: "hi" }], tools: TOOLS })
    });
    check("free no-key model + tools -> 400", c3.status === 400, { status: c3.status, err: c3.body && c3.body.error && c3.body.error.code });
    check("ma loi = tools_not_supported", c3.body && c3.body.error && c3.body.error.code === "tools_not_supported");

    check("nuoc 2: upstream van thay `tools` (khong bi boc khi gui lai)",
      seen.toolsPerCall[1] && seen.toolsPerCall[1].includes("read_file"), seen.toolsPerCall[1]);

    // Khong co tools -> hanh vi cu van chay (khong bi loi vi patch).
    const c4 = await api("/v1/chat/completions", {
      method: "POST",
      body: JSON.stringify({ model: "mock-model", messages: [{ role: "user", content: "khong dung tool" }] })
    });
    check("khong gui tools -> van 200 (khong ban 400 oan)", c4.status === 200, c4.body && c4.body.error);
    check("luot khong gui tools: upstream cung khong thay tools (khong chen ngoai)",
      seen.toolsPerCall[2] === null, seen.toolsPerCall[2]);
