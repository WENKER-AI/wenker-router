...
(async () => {
  ...
  const add = await api('/api/providers/custom/add', {
    ...
    body: JSON.stringify({
      ...
      requiresAuth: false,
      models: [{ id: 'mock-model', name: 'Mock', contextWindow: 4096, isFree: true }],
    }),
  });
  ...
  console.log(
    ...
    'content=' + JSON.stringify(first.j?.choices?.[0]?.message?.content),
    'mockHits=' + hits,
  );
  ...
  console.log(
    ...
    'content=' + JSON.stringify(second.j?.choices?.[0]?.message?.content),
    'cached=' + second.j?.cached,
    'mockHits=' + hits,
  );
  ...
  const pass =
    first.status === 200 &&
    first.j?.choices?.[0]?.message?.content === MARKER &&
    second.status === 200 &&
    second.j?.choices?.[0]?.message?.content === MARKER &&
    hits === 1;
  console.log(
