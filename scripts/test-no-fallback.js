...
function req(method, path, body, headers) {
  return new Promise((resolve) => {
    ...
    const r = http.request({ host: '127.0.0.1', port: 3600, method, path, headers: h }, (res) => {
      let s = ''; res.on('data', (c) => (s += c)); res.on('end', () => { let j = null; try { j = JSON.parse(s); } catch (e) {} resolve({ st: res.statusCode, s, j }); });
    });
...
(async () => {
  ...
  if (!admin) { console.log('KHONG tim thay admin key; /api/keys HTTP ' + keys.st + ' body=' + keys.s.slice(0, 200)); return; }
  ...
  for (const m of list) {
    const r = await req('POST', '/v1/chat/completions', { model: m, messages: [{ role: 'user', content: 'Noi 1 tu thoi.' }], stream: false, max_tokens: 20 });
    ...
    console.log('\n' + m);
    console.log('  HTTP ' + r.st + '  served=' + (r.j && r.j.model) + '  ' + (c ? 'content="' + c.slice(0, 60) + '"' : '') + (err ? '  ERR=' + String(err).slice(0, 160) : ''));
  }

  // Bat lai Smart Fallback nhu cu
  await req('POST', '/api/settings', { enableSmartFallback: s0 && s0.enableSmartFallback !== false }, H);
