...
(async () => {
  ...
  for (const m of [
    'xkiro-basic/mistralai/codestral-2508',
    'xkiro-basic/mistralai/ministral-8b',
    'wenker-vip/wenker-vip-deepseek',
    'wenker-community/wenker-community',
  ]) {
    ...
    const tag =
      r.status === 200 && c && !REFUSAL.test(c)
        ? 'SONG-THAT'
        : r.status === 200 && c
          ? 'FAKE200'
          : 'LOI-' + r.status;
    console.log(
      `  ${tag.padEnd(9)} ${m.padEnd(40)} ${
        c
          ? JSON.stringify(String(c).slice(0, 30))
          : String(e || r.body)
            .replace(/\s+/g, ' ')
            .slice(0, 110)
      }`,
