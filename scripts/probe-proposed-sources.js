...
function req(method, url, bodyObj, headers) {
  return new Promise((resolve) => {
    let u; try { u = new URL(url); } catch (e) { return resolve({ err: "bad url " + e.message }); }
    ...
    const r = https.request({ hostname: u.hostname, port: u.port || 443, path: u.pathname + u.search, method, headers: h },
      (res) => { let b = ""; res.on("data", (c) => (b += c)); res.on("end", () => resolve({ status: res.statusCode, body: b })); });
    r.on("error", (e) => resolve({ err: e.code || e.message }));
    r.setTimeout(20000, () => { r.destroy(); resolve({ err: "timeout" }); });
...
async function probe(name, base) {
  ...
  // 1) DNS
  let ip = "FAIL";
  try { const a = await dns.lookup(host); ip = a.address; } catch (e) { ip = "NXDOMAIN/" + e.code; }
  console.log("  DNS:", host, "->", ip);
  if (ip.startsWith("NXDOMAIN") || ip === "FAIL") { console.log("  ==> domain khong ton tai, bo qua."); return; }
  // 2) /models
  ...
  if (m.err) console.log("  GET /models -> ERR", m.err);
    ...
    console.log(`  GET /models -> HTTP ${m.status}  count=${(mj?.data||mj?.models||[]).length}  mau: ${ids.join(", ") || trim(m.body, 120)}`);
  }
  // 3) chat voi mot vai ten model nghi ngo
  const guessModels = ["deepseek-r1", "deepseek-r1-0528", "deepseek/deepseek-r1-0528:free", "gpt-4o-mini", "llama-3.3-70b-instruct", "default"];
  const firstFromList = (() => { try { const mj = JSON.parse(m.body); return (mj?.data||mj?.models||[])[0]?.id || (mj?.data||mj?.models||[])[0]?.name; } catch { return null; } })();
  const tryList = firstFromList ? [firstFromList, ...guessModels] : guessModels;
  for (const model of tryList) {
    const r = await req("POST", base.replace(/\/+$/, "") + "/chat/completions",
      { model, messages: [{ role: "user", content: "Reply with exactly one word." }], max_tokens: 20 }, {});
    if (r.err) { console.log(`  chat[${model}] -> ERR ${r.err}`); continue; }
    let j; try { j = JSON.parse(r.body); } catch { j = null; }
    const c = j?.choices?.[0]?.message?.content;
    const e = j?.error?.message || j?.message;
    console.log(`  chat[${model}] -> HTTP ${r.status}  ${c ? "OK " + JSON.stringify(trim(c, 40)) : "NO-CONTENT " + trim(e || r.body, 100)}`);
