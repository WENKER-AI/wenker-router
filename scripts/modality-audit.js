...
function modalityLabel(p) {
  ...
  if (/(tts|elevenlabs|cartesia|neets|coqui|bark|musicgen|audiocraft|\/speech|text-to-speech|\/audio|\/music)/.test(base)) return "AUDIO";
...
const SUSPECT = /(image|video|audio|speech|tts|voice|music|suno|playground|dalle|imagen|firefly|stability|replicate|bfl|luma|pika)/i;
const suspectChat = notFlagged.filter((p) => SUSPECT.test(String(p.baseUrl) + " " + String(p.name)));
