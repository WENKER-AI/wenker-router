...
(async () => {
  ...
  const byProv = {};
  for (const m of data) { const pid = m.id.split("/")[0]; byProv[pid] = (byProv[pid] || 0) + 1; }
  ...
  const out = {
    ...
    wenkerTierModels: Object.fromEntries(Object.entries(byProv).filter(([k]) => k.startsWith("wenker") || k === "openrouter")),
    aliveProviders: Object.entries(health.json || {}).filter(([, v]) => v && v.ok === true).map(([k]) => k),
    downProviders: Object.entries(health.json || {}).filter(([, v]) => v && v.ok === false).map(([k]) => k),
  };
