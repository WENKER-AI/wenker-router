...
const looksDead = (c) =>
  /reached its budget|raise the budget|unavailable for free|no endpoints found|invalid api key/i.test(
    String(c || ''),
  );
...
(async () => {
  ...
  for (const c of CANDIDATES) {
    ...
    console.log(
      `${ok ? 'SONG ' : 'CHET '} ${c.slug.padEnd(34)} HTTP ${r.status}  ${
        content
          ? JSON.stringify(String(content).slice(0, 40))
          : String(j?.error?.message || r.body)
            .replace(/\s+/g, ' ')
            .slice(0, 90)
      }`,
    );
  ...
  const models = live.map((c) => ({
    id: c.slug,
    name: c.name,
    contextWindow: c.free ? 32000 : 128000,
    isFree: Boolean(c.free),
  }));
  ...
  for (const id of [models[0].id, 'openrouter/' + models[0].id]) {
    ...
    const r = await call('/v1/chat/completions', A, {
      model: id,
      messages: [{ role: 'user', content: 'one word: ' + nonce }],
      max_tokens: 20,
    });
    let j;
    try {
    ...
    const content = j?.choices?.[0]?.message?.content;
    console.log(
      `  ${content && !looksDead(content) ? 'OK ' : '!! '} model="${id}" -> HTTP ${r.status} ${content ? JSON.stringify(String(content).slice(0, 30)) : JSON.stringify(String(j?.error?.message || '').slice(0, 90))}`,
    );
  }
  ...
  console.log(
    `  /v1/models tong=${ids.length}  mo-dau-openrouter=${ids.filter((x) => x.startsWith('openrouter/')).length}  [${ids.filter((x) => x.startsWith('openrouter/')).join(', ')}]`,
  );
})();
