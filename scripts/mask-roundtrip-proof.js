...
(async () => {
  ...
  check(
    'co key that tren dia (wenker-cloud)',
    Boolean(original),
    original ? original.slice(0, 7) + '...' + original.slice(-4) : 'EMPTY',
  );
  ...
  check(
    'key THAT VAN NGUYEN sau khi gui mask (khong bi de ghi de)',
    after1 === original,
    after1 === original ? 'giu duoc' : `BI MANG: ${JSON.stringify(after1).slice(0, 40)}`,
  );
  check(
    'phan response khong lo plaintext',
    !r1.text.includes(original),
    r1.json && r1.json.provider && r1.json.provider.userApiKey,
  );

  // 2. Doi truong KHAC cung luc (baseUrl) -> van phai luu
  check(
    'truong khac van luu duoc',
    /pollinations/.test(
      realKeyOnDisk()
        ? JSON.stringify(
          JSON.parse(fs.readFileSync(CFG, 'utf8')).providerOverrides['wenker-cloud'].baseUrl || '',
        )
        : '',
    ),
    'baseUrl giu nguyen',
  ...
  check(
    'mask gia (dui tail) cung bi bo qua, khong ghi de',
    r3.status === 200 && realKeyOnDisk() === original,
    realKeyOnDisk() === original ? 'an toan' : 'BI MANG',
  );

  // 4. Gui key hop le (khong chua '•') -> PHAI luu that
  const probeKey = 'sk-wenker-roundtrip-probe-0001';
  const r4 = await api('/api/providers/wenker-cloud', {
    method: 'POST',
    body: JSON.stringify({ userApiKey: probeKey }),
  });
  check(
    'gui key that -> luu dung',
    r4.status === 200 && realKeyOnDisk() === probeKey,
    realKeyOnDisk().slice(0, 12),
  );

  // 5. Xoa: gui chuoi rong
  const r5 = await api('/api/providers/wenker-cloud', {
    method: 'POST',
    body: JSON.stringify({ userApiKey: '' }),
  });
  check(
    'gui chuoi rong -> xoa het key',
    r5.status === 200 && realKeyOnDisk() === '',
    JSON.stringify(realKeyOnDisk()),
  );

  // 6. KHOI PHUC key ban dau cua user
  const r6 = await api('/api/providers/wenker-cloud', {
    method: 'POST',
    body: JSON.stringify({ userApiKey: original }),
  });
  check(
    'khoi phuc key ban dau',
    r6.status === 200 && realKeyOnDisk() === original,
    realKeyOnDisk() ? 'ok' : 'KHONG KHOI PHUC DUOC',
  );
  ...
  check(
    'GET /api/providers: wenker-cloud mask + hasApiKey',
    wc && wc.hasApiKey === true && /\u2022{4}/.test(String(wc.userApiKey)),
    `hasApiKey=${wc && wc.hasApiKey} key=${wc && wc.userApiKey}`,
  );
