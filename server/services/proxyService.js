...
class ProxyService {
    ...
    if (model.includes('/')) {
      ...
      if (provider) {
        ...
        const found = (provider.models || []).find(
          (m) => m.id === mName || m.targetModel === mName,
        );
        return {
          provider,
          targetModel: found?.targetModel || mName,
          requested: model,
          known: true,
          modelKnown: Boolean(found),
        };
      }
    ...
    for (const candidate of [model, bare]) {
      for (const p of providers) {
        ...
        if (found) {
          return {
            provider: p,
            targetModel: found.targetModel || candidate,
            requested: model,
            known: true,
            modelKnown: true,
          };
    ...
    if (bare && bare !== model) {
      for (const p of providers) {
        ...
        if (found) {
          return {
            provider: p,
            targetModel: found.targetModel || found.id,
            requested: model,
            known: true,
            modelKnown: true,
          };
  ...
  async _directChat(provider, targetModel, messages, options = {}) {
    ...
    if (!canExecute.allowed) {
      const error = new UpstreamError(
        `${provider.name}: Circuit breaker is OPEN. ${canExecute.reason || 'Provider temporarily unavailable'}`,
        503,
        'circuit_open'
      );
    ...
    try {
      ...
      if (provider.requiresAuth && !apiKey)
        throw new Error(`${provider.name}: chua cau hinh API Key`);
      ...
      if (!r.ok) {
        ...
        const e = new UpstreamError(
          `${provider.name} HTTP ${r.status}: ${t.slice(0, 160)}`,
          r.status,
        );
      ...
      // Đảm bảo failure được ghi nhận
      if (!err.code || err.code !== 'CIRCUIT_OPEN') {
      ...
      try {
        ...
        if (freeProxyService.isPollinationsProvider(provider)) {
          const data = await freeProxyService.handlePollinations({
            model: id === 'wenker-cloud' ? 'wenker-deepseek-v3-free' : id,
            messages,
            stream: false,
            targetModel:
  ...
  _modalityLabel(provider) {
    ...
    if (/(transcription|deepgram|assemblyai|speechmatics|whisper)/.test(base))
      return 'nhận dạng giọng nói (STT)';
    if (
      /(tts|elevenlabs|cartesia|neets|coqui|bark|musicgen|audiocraft|\/speech|text-to-speech|\/audio|\/music)/.test(
        base,
      )
    )
      return 'âm thanh (TTS/audio)';
  ...
  async handleChatCompletion({ req, res, body, wenkerKey }) {
    ...
    if (
      hasTools &&
      toolGuardProvider &&
      (freeProxyService.isPollinationsProvider(toolGuardProvider) ||
        toolGuardProvider.id === 'duckduckgo')
    ) {
      db.addLog({
    ...
    if (cached) {
      ...
      return res.json({
        ...
        usage: {
          prompt_tokens: cPrompt,
          completion_tokens: cCompletion,
          total_tokens: cPrompt + cCompletion,
        },
        cached: true,
      });
    }
    ...
    try {
      ...
      if (freeProxyService.isPollinationsProvider(provider)) {
        ...
        db.addLog({
          ...
          ...servedBy !== provider.id
            ? { fallbackFrom: provider.id, fallbackReason: 'upstream_down' }
            : {},
          status: 200,
          latencyMs: latency,
          promptTokens,
        ...
        try {
          ...
          if (stream) {
            ...
            result = {
              streamed: true,
              model: fb.data.model,
              promptTokens: this._estimatePromptTokens(messages),
              completionTokens: this._estimateTokens(fbContent),
            };
          } else {
            result = fb.data;
          }
      ...
      const apiKey =
        provider.userApiKey ||
        (req.headers.authorization && !req.headers.authorization.includes('sk-wenker')
      ...
      if (!upstreamRes.ok) {
        ...
        if (settings.enableSmartFallback && !res.headersSent && !hasTools) {
          console.log(
            `[Failover] Provider ${provider.name} failed with ${upstreamRes.status}. Falling back to WENKER Cloud.`,
          );
          ...
          try {
            ...
            const fbCompletion =
              fb.data.usage?.completion_tokens || this._estimateTokens(fbContent);
            if (stream) {
              await freeProxyService._writeSse(res, fb.data.model || model, fbContent);
            } else {
      ...
      if (stream) {
        ...
        if (data.choices && !hasTools)
          db.cacheSet(cacheKey, data.choices?.[0]?.message?.content || '', data.usage);
      ...
      const status = [401, 402, 403, 404, 408, 429, 502, 503, 504].includes(upstreamStatus)
        ? upstreamStatus
        : 500;
      return res.status(status).json({
        error: {
          ...
          hint:
            err.hint ||
            'Thu lai sau it phut, hoac mo tab \'Nha Cung Cap\' de nhap API Key mien phi (OpenRouter/Groq/Gemini/NVIDIA) va chuyen model sang provider do.',
        },
      });
    }
  }
  ...
  async handleAnthropicMessages({ req, res, body, wenkerKey }) {
    ...
    if (
      anthMetered &&
      !quota.tryConsume(anthQuotaSubject, this._estimatePromptTokens(normalized))
    ) {
        ...
        status: (code) => ({
          json: (errBody) => {
            ...
            res
              ...
              .json({
                type: 'error',
                error: { type: 'api_error', message: errBody?.error?.message || 'Upstream error' },
              });
          },
        }),
        write: (chunk) => {
      ...
      if (!mockRes.headersSent && !res.headersSent) {
        res.status(502).json({
          ...
          error: {
            type: 'api_error',
            message:
              'Nhà cung cấp không trả về nội dung nào. Vui lòng thử lại hoặc chọn model khác.',
          },
        });
      }
    }
    ...
    try {
      ...
      if (!testUrl || !/^https?:\/\//i.test(testUrl)) {
        return {
          success: false,
          latencyMs: 0,
          error: 'Base URL không hợp lệ (chưa cấu hình)',
          providerId,
        };
      }
      if (testUrl.includes('{')) {
        return {
          success: false,
          latencyMs: 0,
          error: 'Base URL chứa biến template, không ping được. Hãy điền giá trị thật.',
          providerId,
        };
      }
