...
(function () {
  ...
  function makeSnake() {
    ...
    return {
      ...
      step: function () {
        ...
        if (head.x < 0 || head.y < 0 || head.x >= W || head.y >= H || inside(head)) {
          alive = false;
          return;
        }
        snake.unshift(head);
  ...
  function makeCatch() {
    ...
    return {
      ...
      step: function () {
        spawn += 1;
        if (spawn % 14 === 0)
          fishes.push({ x: rand(W - 2) + 1, y: 0, v: 0.5 + Math.random() * 0.7 });
        for (var i = fishes.length - 1; i >= 0; i--) {
          var f = fishes[i];
          f.y += f.v;
          if (f.y >= H - 6 && f.x > basket.x - 1 && f.x < basket.x + basket.w + 1) {
            fishes.splice(i, 1);
            score += 1;
            continue;
          }
      ...
      draw: function (c) {
        c.fillStyle = COL.bg;
        c.fillRect(0, 0, W, H);
        c.fillStyle = 'rgba(74,168,255,0.10)';
        c.fillRect(0, H - 5, W, 5);
        c.fillStyle = COL.whale;
        fishes.forEach(function (f) {
          var y = Math.floor(f.y);
          c.fillRect(f.x - 1, y, 3, 1);
          c.fillRect(f.x, y - 1, 1, 1);
        });
        c.fillStyle = COL.green;
  ...
  function makeFlappy() {
    ...
    return {
      ...
      step: function () {
        ...
        if (frame % 60 === 0) {
          gapY = 12 + rand(H - 24);
          pipes.push({ x: W + 6, gap: gapY, passed: false });
        }
        for (var i = pipes.length - 1; i >= 0; i--) {
          var p = pipes[i];
          p.x -= 0.9;
          if (bird.x + bird.r > p.x && bird.x - bird.r < p.x + 5) {
            if (bird.y - bird.r < p.gap - GAP / 2 || bird.y + bird.r > p.gap + GAP / 2)
              alive = false;
          }
          if (!p.passed && p.x + 5 < bird.x) {
            p.passed = true;
            score += 1;
          }
      ...
      draw: function (c) {
        ...
        c.fillRect(bird.x - 2, by, 5, 2);
        c.fillRect(bird.x - 1, by - 1, 3, 4);
        hud(c, this.label + '  ·  ' + score);
        if (!alive) over(c);
      },
    };
  ...
  function Mount(canvas, listEl) {
    ...
    function pointerDown(e) {
      ...
      if (current.tap) current.tap();
      else if (current.key) current.key(e.pointerType === 'mouse' ? 'flap' : 'flap');
  ...
  window.WenkerGames = {
    mount: function (canvas, list) {
      return Mount(canvas, list);
    },
  };
})();
